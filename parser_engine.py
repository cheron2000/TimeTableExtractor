...
SKIP_TOKENS: frozenset[str] = frozenset({
    ...
    "A", "B", "C", "D", "E", "F", "G", "H",
    # Subject / department names that appear in cells
    "CHEM", "PHY", "MECH", "CIVIL", "COMP", "MTRX", "BEE", "EM", "EG",
    "PPS", "IKS", "M2", "M1", "BXE", "MPW", "DTI", "DS", "AI", "RO",
    "NSS", "NCC", "TUT", "ARTS", "FINE", "FIRE", "SELF",
    # Header / metadata words
    "FE", "SE", "TE", "BE", "YEAR", "FIRST", "NAME", "MR", "MRS", "DR",
    "OF", "RM", "EXTRA", "VIDYA", "WOMAN", "PATIL", "THURS",
})

# Tokens to strip from subject strings (structural/noise words only, NOT subject names)
SUBJECT_STRIP_TOKENS: frozenset[str] = frozenset({
    "MON", "TUE", "WED", "THU", "FRI", "SAT", "SUN",
    "THE", "AND", "FOR", "NOT", "ARE", "WAS", "HAS",
    "LAB", "LEC", "TH", "DAY", "TIME", "AM", "PM",
    "NO", "ID", "PR", "TD", "LE", "ROOM", "CLASS",
...
def normalise_time(raw: str) -> str:
    ...
    if not raw:
        return ""
    # Handle dot-separated times: 9.3 → 9:30, 10.45 → 10:45, 1.3 → 1:30
    dot_m = re.match(r'^(\d{1,2})\.(\d{1,2})$', raw)
    if dot_m:
        h  = int(dot_m.group(1))
        mn = dot_m.group(2).ljust(2, '0')  # "3" → "30", "45" → "45"
        return f"{h}:{mn}"
...
def clean_subject(raw_seg: str, remove_codes: list[str]) -> str:
    ...
    s = re.sub(r'\(\s*\)', '', s)  # remove empty parens before _clean_paren
    s = re.sub(r'\(([^)]*)\)', _clean_paren, s)
    s = re.sub(r'\(\s*\)', '', s)  # remove any remaining empty parens
    s = re.sub(r'^[\s\-:\,–—]+', '', s)
    s = re.sub(r'[\s\-:\,–—]+$', '', s)
    # Only strip structural noise words, not subject names
    skip_pattern = r'\b(?:' + '|'.join(re.escape(t) for t in sorted(SUBJECT_STRIP_TOKENS, key=len, reverse=True)) + r')\b'
...
def _clean_paren(m: re.Match) -> str:
    ...
    # Strip parens that contain only: codes, batch refs, room numbers, single letters
    if all(
        re.match(r'^[A-Z]{2,5}$', t)
        or re.match(r'^[A-H]\d$', t)
        or re.match(r'^\d+[A-Z]?$', t)
        or re.match(r'^[A-Z]$', t)
        or not t
        for t in toks
    ):
...
def _entry_is_lab(compact: str) -> bool:
    """Entry is a lab if it contains any lab keyword or lab room pattern."""
    return bool(re.search(r'\blab\b', compact, re.IGNORECASE))
...
class TimetableParser:
    ...
    def _detect_class_cols(self):
            ...
            if row_val in {"", "CLASS", "DIVISION", "DAY", "FE"}:
                for c, val in enumerate(row):
                    ...
                    if v and not v.isdigit():
                        ...
                        vu = v.upper()
                        # Normalise: "FE - A (Comp)" → match div A
                        # Strip spaces/dashes around the letter
                        for div in DIVISION_CONFIG:
                            patterns = [
                                vu == div,
                                vu == f"FE-{div}",
                                vu == f"FE {div}",
                                vu.startswith(f"FE-{div} "),
                                vu.startswith(f"FE - {div} "),
                                vu.startswith(f"FE - {div}("),
                                f"({div})" in vu,
                                vu.startswith(f"{div} ("),
                                # Match "FE - A (Comp)" style
                                bool(re.search(rf'\bFE\s*[-–]\s*{div}\b', vu)),
                            ]
                            if any(patterns):
        ...
        log.info(f"[div_col_map] { {k: list(v) for k, v in self.div_col_map.items()} }")
    ...
    def _build_day_row_map(self):
        ...
        seen_slots: dict[str, set[str]] = {d: set() for d in FIXED_DAYS}
        ...
        for r, row in enumerate(self.grid):
            ...
            if not cur_day or cur_day not in FIXED_DAYS:
                continue

            # Skip rows where time cell is a merged continuation (already recorded)
            if self._is_merged_continuation(r, self.time_col):
                continue
            ...
            if not slot_key:
                continue
            # Avoid duplicate slot entries for the same day (e.g. WED appearing twice)
            if slot_key not in seen_slots[cur_day]:
                seen_slots[cur_day].add(slot_key)
        ...
        for row in self.grid:
            for cell in row:
                ...
                for code in extract_teacher_codes(cell):
                    ...
                    elif code not in SKIP_TOKENS and 2 <= len(code) <= 3:
                        occ[code] += 1
        for code, count in occ.items():
            if count >= 4:  # raised from 2 — reduces false positives
                self._known_teachers.add(code)
    ...
    def _merge_labs(self, schedule: dict) -> dict[str, dict]:
        """
        Mark lab slots: the first slot gets span=2, the next is skipped.
        Frontend renders the first as a wide cell spanning both time columns.
        """
        merge_info: dict[str, dict] = {d: {} for d in FIXED_DAYS}
        for day in FIXED_DAYS:
            for i, key in enumerate(LECTURE_SLOTS):
                entries = schedule[day].get(key, [])
                if not entries:
                    continue
                if any(e["type"] == "lab" for e in entries) and i + 1 < len(LECTURE_SLOTS):
                    next_key = LECTURE_SLOTS[i + 1]
                    merge_info[day][key]      = {"span": 2}
                    merge_info[day][next_key] = {"skip": True}
                    for e in entries:
                        e["span"] = 2
        return merge_info
